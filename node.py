...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for i in possible_splits:
            if i != 0:
                ...
                gini_left = 1 - (left_ones / i) ** 2 - (left_zeros / i) ** 2
                gini_right = 1 - (right_ones / (len(y) - i)) ** 2 - (right_zeros / (len(y) - i)) ** 2
                gini_gain = 1 - gini_left * i / len(y) - gini_right * ((len(y) - i) / len(y))
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is None:
            feature_subset = int(math.sqrt(X.shape[1]))
